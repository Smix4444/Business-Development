  ...
  try {
    if (req.method === 'GET') {
      if (payload.role === 'student') {
        const apps = await db.getApplicationsByStudent(payload.userId);
      ...
      if (payload.role === 'company') {
        const apps = await db.getApplicationsByCompany(payload.userId);
        return res.json(apps.map(a => ({
          id: a.id,
          message: a.message,
          status: a.status,
    ...
    if (req.method === 'POST') {
      ...
      const internship = await db.findInternshipById(parseInt(internshipId));
      if (!internship) return res.status(404).json({ error: 'Internship not found' });

      const existing = await db.findApplication(payload.userId, parseInt(internshipId));
      if (existing) return res.status(409).json({ error: 'Already applied' });

      const app = await db.createApplication({ studentId: payload.userId, internshipId: parseInt(internshipId), message });
